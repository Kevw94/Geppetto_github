...
namespace MikeNspired.XRIStarterKit
{
    public class FlashLight : MonoBehaviour
    {
        ...
        {
            Debug.Log(" Bouton next color pressé !");
            if (_currentColor == 5)
            {
                SetColor(0);
            }
            ...
            {
                SetColor(_currentColor + 1 );
            }
        }

        private void Awake()
        {
            // Essaie de trouver le bouton rouge dans les enfants
            XRSimpleInteractable button = GetComponentInChildren<XRSimpleInteractable>(true);
            if (button != null)
            {
                button.selectEntered.RemoveAllListeners();
                button.selectEntered.AddListener(_ => SetNextColor());
                Debug.Log("Bouton rouge lié automatiquement à la lampe");
            }
            else
            {
                Debug.LogWarning("Aucun XR Simple Interactable trouvé dans la lampe !");
            }
        }
    }
}